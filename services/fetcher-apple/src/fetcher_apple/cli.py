...
from fetcher_apple.api import list_playlists
from fetcher_apple.download import DownloadError, fetch_playlist
...
def _cmd_fetch(args: argparse.Namespace) -> int:
    ...
    entry = next(
        (
            ...
            if p.name == args.playlist and p.source == "apple_music"
        ),
        None,
    )
    if entry is None:
        print(
            f"ERROR: no apple_music playlist named {args.playlist!r} "
    ...
    if result.unmatched_paths:
        print(f"unmatched (downloaded but not tagged/recorded): {len(result.unmatched_paths)}")
        for p in result.unmatched_paths:
            print(f"  {p}")
    if result.failed_tracks:
        print(f"failed to download: {len(result.failed_tracks)}")
        for t in result.failed_tracks:
            print(f"  {t.source_id} - {t.artist} - {t.title}")
    return 0


def main() -> None:
    parser = argparse.ArgumentParser(prog="fetcher-apple")
    subparsers = parser.add_subparsers(dest="command", required=True)

    list_parser = subparsers.add_parser(
        "list-playlists", help="List the account's Apple Music library playlists"
    )
    list_parser.add_argument("--cookies-path", required=True)
    ...
    fetch_parser.add_argument("--cookies-path", required=True)
    ...
    fetch_parser.add_argument(
        "--lock-path",
        default=None,
        help="Path to the Apple Music session lock file. Defaults to "
        ".apple_music.lock next to --state-path.",
    )
    fetch_parser.add_argument(
        "--lock-timeout",
        type=float,
